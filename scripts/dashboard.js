...
(async () => {
  try {
    ...
    if (
      !data.logado ||
      (data.perfil !== "admin" && data.perfil !== "desenvolvedor")
    ) {
...
function renderRecentes(lista) {
  ...
  if (!lista || lista.erro || !Array.isArray(lista) || lista.length === 0) {
    el.innerHTML =
      '<div class="recent-loading" style="color:rgba(255,255,255,0.2);font-size:13px;letter-spacing:1px;">Nenhum empréstimo ativo</div>';
  ...
  el.innerHTML = lista
    .map((item) => {
      ...
      const devolucao = item.data_devolucao
        ? new Date(item.data_devolucao + "T00:00:00").toLocaleDateString(
            "pt-BR",
          )
        : "—";
      return `
